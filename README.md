...
8. everything above would *just replay* once your system boot up again.


### Complain
1. I believe that it is us human beings who use computers for work, game or whatever else they prefer, rather than computer programs to confuse or 'play' humans. It is true at least in the year 2022 and the past.
2. I have no idea why applications tend to **steal** input focus from the user, and why *window system* of OS could allow so. It is **disruptive** and **show no respect** to users, I mean human beings, to be interrupted for trivial software events when they're concentrating on one specific application.
...
4. Maybe we should also blame the *window system* who give up focus too easily.
...
### Prerequisites
Again, this is a **dirty and brute-force hack** into the *window system* of **Ubuntu 20.04** to **solve focus-stealing issues**. Use this solution **AT YOUR OWN RISK**. Prerequisites as follows.
...
* knowledge & skills
	...
	* Use experience in *Gnome Shell* graphical interface, which is the default *desktop environment* for Ubuntu 18.04/20.04
...
7. Do configuration
	...
	* Since *libmutter* is hosted in *gnome-shell*, you should set it to `/usr/bin/gnome-shell`
			...
			echo 'X_WINDOW_MANAGER_HOST=/usr/bin/gnome-shell' >> ~/.profile
...
2. Since this hack **violates** *ICCCM/EWMH protocol*, some applications may failed to get focus or even crash. I haven't observed this issue on any *normal* application I've used, but there might be.
3. There're still some applications who could **steal focus** even with *libmutter* hack applied. Those *toxic and brute* programs **directly send focus message** to *Xorg server* via either `XSetInputFocus` from **libx11** or `xcb_set_input_focus` from **libxcb**.
	* **Java** from *OpenJDK*. It calls `XSetInputFocus` on **every** dialog it creates. So nearly **every** java application with a main window would **steal your focus forcefully**. You can apply the *libx11* hack to block this. However some text fields **failed to receive inputs** after applying this hack on *Ubuntu 18.04*.
	* **VirtualBox**. Actually it's **fine** for *VirtualBoxVM* to mess with *Xorg*. When you're working in a VM, you want your input go directly into that VM, so *VirtualBoxVM* grabbed keyboard & mouse for you. However, a call to `xcb_set_input_focus` **ruins everything**. Imagine you launch a VM and quickly switch to another window. When VM window first show up or when *guest OS* changes resolution, **input focus** would be stolen by the window, and *window manager* even don't know about this. What's worse, on focus, *VirtualBoxVM* would by default **install keyboard & mouse grabs** into *Xorg*. As a result, your keyboard suddenly **stopped working** since it's grabbed into the VM, and you cannot tell why if the VM window isn't visible, you have to press the *Host key* to release the keyboard grab. It's really **confusing and annoying** for people not familiar with *VirtualBox*.
	* **Wine**. I haven't inspected the source code of *wine*, but since it acts like a *Windows API translator*, there must be very crazy code struggling to meet Windows behavior. The result is, after applying the hack, many Windows applications **failed to receive inputs**, many others **still steal focus**. This also applies to *Win32-based games* launched using *Steam Proton*.

### Q&A
1. Is the hack **safe** ?
	* If I say *yes*, would you believe me? Go and check the code yourself, both **mutter** and this project are **open source**
...
3. Why not submit patch/feature-request to respective *development groups* ?
	* It's actually a **hack** than a patch. It violates *ICCCM/EWMH protocol*, mess up the base code and make libraries *judge* its users, just to introduce weird functionality beneficial to a few people but confuse most normal users if it could ever became a setting.
